...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    try {
      ...
      // Traditional session failed, try OAuth
      console.log("[Auth] Traditional session failed, trying OAuth:", String(error));
    }

    // Try OAuth session (solo si parece un JWT - 3 partes separadas por puntos)
      ...
      try {
        ...
        if (!oauthUser) {
          try {
            ...
            const fetchedOAuthUser = await db.getUserByOpenId(userInfo.openId);
            if (fetchedOAuthUser) {
              user = fetchedOAuthUser;
            }
          } catch (error) {
        ...
        console.error("[Auth] OAuth verification failed:", error);
        throw ForbiddenError("Invalid session");
      }
    } else {
      // Not a JWT, so it's not a valid OAuth session
      throw ForbiddenError("Invalid session cookie");
    }
