...
function computeBusinessDashboardData({
  ...
  for (const unit of soldUnits) {
    ...
    const sale = salesList.find((s: any) => 
      saleItemsList.some((si: any) => si.saleId === s.id && si.unitId === unit.id)
    );
  ...
  const totalRevenue = soldUnits.reduce((s: number, u: any) => {
    const si = saleItemsList.find((item: any) => item.unitId === u.id);
  ...
  const operationalExpenses = expensesList
  ...
  const inventoryValue = unitsList
  ...
  const purchased = unitsList.length;
  const inRepair = unitsList.filter((u: any) => u.status === "in_repair").length;
  const available = unitsList.filter((u: any) => u.status === "available").length;
  const sold = soldUnits.length;

  // 5. Gastos operativos por categoría
  const expensesByCategory = expensesList
    .filter((e: any) => new Date(e.expenseDate || e.createdAt) >= monthStart)
  ...
  const repairsCompleted = repairsList.filter((r: any) => 
    r.status === "completed" && new Date(r.endDate || r.updatedAt || r.createdAt) >= monthStart
  ).length;
  
  const avgRepairDays = repairsList
  ...
  const lossRepairs = repairsList
    .filter((r: any) => {
      if (r.status !== "completed") return false;
      const unit = unitsList.find((u: any) => u.id === r.unitId);
  ...
  const avgInventoryDays = soldUnits
    .map((u: any) => {
      const sale = salesList.find((s: any) => 
        saleItemsList.some((si: any) => si.saleId === s.id && si.unitId === u.id)
      );
  ...
  // Equipos +60 días
  const oldInventory = unitsList.filter((u: any) => {
  ...
  for (let i = 7; i >= 0; i--) {
    ...
    const weekSales = salesList.filter((s: any) => {
...
export const dashboardRouter = router({
  ...
  getBusinessDashboard: protectedProcedure.query(async ({ ctx }) => {
    ...
    if (!db) {
      ...
      // ═══ MODO REAL (DATABASE) ═══
      const [dbUnits, dbRepairs, dbSales, dbSaleItems, dbExpenses] = await Promise.all([
        db.select().from(units),
        db.select().from(repairs),
        db.select().from(sales).where(ne(sales.status, "cancelled")),
        db.select().from(saleItems),
        db.select().from(financialTransactions).where(eq(financialTransactions.type, "expense")),
      ]);
      unitsList = dbUnits;
      repairsList = dbRepairs;
      salesList = dbSales;
      saleItemsList = dbSaleItems;
      expensesList = dbExpenses;
    }
